...
export default class ApgarPage {
        ...
        this.data = {
            ...
            reference: {
                ...
                paragraphs: [
                    ...
                    "Аббревиатура APGAR является мнемоникой: Appearance (цвет кожи), Pulse (ЧСС), Grimace (рефлекторная возбудимость), Activity (мышечный тонус), Respiration (дыхание)."
                ],
                ...
                formulas: [
                    { 
                        name: 'Сумма Апгар', 
                        formula: 'A + P + G + A + R (макс 10 баллов)', 
                        example: '2+2+2+2+2 = 10 — норма' 
                    },
                    { 
                        name: 'Динамика', 
                        formula: 'Δ = Апгар 5 мин − Апгар 1 мин', 
                        example: '7 − 4 = +3 → хорошая динамика реанимации' 
                    },
                    { 
                        name: 'Критерии тяжёлой асфиксии', 
                        formula: 'Апгар ≤ 3 на 1-й минуте', 
                        example: 'Требуется интенсивная СЛР новорождённого' 
                    }
                ],
                quickRules: [
                    { icon: '✅', rule: '8-10 баллов — нормальное состояние, наблюдение' },
                    { icon: '⚠️', rule: '4-7 баллов — асфиксия средней степени, O₂, стимуляция' },
                    { icon: '🚨', rule: '1-3 балла — тяжёлая асфиксия, ИВЛ, реанимация' },
                    { icon: '💀', rule: '0 баллов — клиническая смерть, экстренная СЛР' },
                    { icon: '📈', rule: 'Улучшение на 5-й минуте — хороший прогностический признак' },
                    { icon: '🔁', rule: 'При ≤ 7 на 5-й мин → оценка на 10, 15, 20 мин' }
                ],
                examples: [
                    {
                        scenario: 'Здоровый новорождённый: розовый, ЧСС 140, кричит, активные движения, хорошее дыхание',
                        calculation: 'Апгар = 2+2+2+2+2 = 10/10 (1 мин) → 10/10 (5 мин) → норма, без динамики'
                    },
                    {
                        scenario: 'Асфиксия в родах: цианоз, ЧСС 80, слабая гримаса, сниженный тонус, нерегулярное дыхание',
                        calculation: 'Апгар 1 мин = 1+1+1+1+1 = 5 → стимуляция, O₂. Апгар 5 мин = 2+2+2+2+2 = 10 → хорошая динамика (+5)'
                    },
                    {
                        scenario: 'Тяжёлая асфиксия: бледность, ЧСС 60, нет реакции, атония, апноэ',
                        calculation: 'Апгар 1 мин = 0+1+0+0+0 = 1 → ИВЛ, компрессии, адреналин. Апгар 5 мин = 4 → продолжаем реанимацию'
                    }
                ]
            },
            timePoints: [
            ...
            groups: [
                {
                    id: 'A', title: 'Appearance — Цвет кожи', shortTitle: 'A',
                    fullName: 'Цвет кожи', icon: 'palette',
                    items: [
                ...
                {
                    id: 'P', title: 'Pulse — Частота сердечных сокращений', shortTitle: 'P',
                    fullName: 'ЧСС', icon: 'monitor_heart',
                    items: [
                ...
                {
                    id: 'G', title: 'Grimace — Рефлекторная возбудимость', shortTitle: 'G',
                    fullName: 'Рефлексы', icon: 'sentiment_dissatisfied',
                    items: [
                ...
                {
                    id: 'A2', title: 'Activity — Мышечный тонус', shortTitle: 'A',
                    fullName: 'Тонус', icon: 'accessibility_new',
                    items: [
                ...
                {
                    id: 'R', title: 'Respiration — Дыхание', shortTitle: 'R',
                    fullName: 'Дыхание', icon: 'air',
                    items: [
    ...
    async render() {
        ...
        this.container.innerHTML = `
            <div class="page-content calc-page apgar-page">
    ...
    renderResult() {
        ...
        const inlineStyle = getResultInlineStyle(colorClass);

        // Интерпретация последней оценки
        const interpretationHtml = lastRange ? `
            <div class="apgar-interpretation">
                <strong>${lastRange.label}:</strong> ${lastRange.description}
            </div>
        ` : '';

        return `
            <div class="result-content result-${colorClass} apgar-result" style="${inlineStyle}">
                <div class="apgar-result-grid">
                    ...
                    <div class="apgar-result-divider">
    ...
    updateSelections() {
        ...
        this.data.groups.forEach(group => {
            ...
            this.container.querySelectorAll(`.gcs-radio-item[data-group="${group.id}"]`).forEach(item => {
    ...
    setupEventListeners() {
        ...
        this._clickHandler = (e) => {
            ...
            // Переключатель времени
            const timeBtn = e.target.closest('.apgar-time-btn');
            if (timeBtn && this.container.contains(timeBtn)) {
                e.preventDefault();
                this.currentTime = timeBtn.dataset.time;
                this.container.querySelectorAll('.apgar-time-btn').forEach(b => {
                    b.classList.toggle('active', b === timeBtn);
                });
                this.updateSelections();
                return;
            }
    ...
    updateTimeButtonScores() {
        this.data.timePoints.forEach(tp => {
            const btn = this.container.querySelector(`.apgar-time-btn[data-time="${tp.id}"]`);
            if (btn) {
                const score = this.calculateScore(tp.id);
                const count = this.countFilled(tp.id);
