...
export default class GuidelinesPage {
        ...
        this.container.innerHTML = `
            <div class="page-content">
                ...
                <div style="margin-bottom: 12px;">
                    <input type="search" id="guidelines-search" placeholder="Поиск по названию, симптомам..." autocomplete="off">
                </div>
    ...
    renderList(items) {
        ...
        return items.map(item => {
            ...
            const symptoms = (item.symptoms || []).slice(0, 3).map(s => `
                    ...
                    <div style="display: flex; align-items: flex-start; gap: 12px;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div class="card-subtitle" style="margin-bottom: 6px;">
                                ...
                                ${symptoms ? `<div class="category-badges" style="margin-top: 6px;">${symptoms}</div>` : ''}
                            </div>
                            ${item.summary ? `<div class="card-body">${item.summary}</div>` : ''}
                        </div>
    ...
    _setupEventListeners() {
        const searchEl = document.getElementById('guidelines-search');
    ...
    _setupItemListeners() {
        document.querySelectorAll('.guideline-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                const guideline = this.engine.items.find(g => g.id == id);
                if (guideline) {
                    // Добавляем в недавние
                    storage.addRecent({
                        id: `guideline-${guideline.id}`,
                        title: guideline.title,
                        subtitle: (guideline._categories || []).join(', '),
