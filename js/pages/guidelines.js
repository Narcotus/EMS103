...
export default class GuidelinesPage {
    ...
    async render() {
        ...
        // ✅ Сохраняем страницу в историю с source
        try {
            storage.addRecent({
                id: 'guidelines',
                ...
                icon: 'clinical_notes',
                section: 'guidelines',
                source: 'guidelines'  // ✅ ДОБАВЛЕНО для цветной иконки
            });
        } catch (e) {
        ...
        try {
            ...
            console.error('❌ Ошибка загрузки протоколов:', e);
            this.engine.setItems([]);
            window.showSnackbar?.('Не удалось загрузить протоколы');
        }
    ...
    renderFullPage() {
        ...
        this.container.innerHTML = `
            <div class="page-content guidelines-page">
                <div class="page-header">
                    <h1>Клинические рекомендации</h1>
                    <p>Протоколы и алгоритмы · ${this.engine.items.length} записей</p>
                </div>

                <div class="guidelines-search-wrapper">
                    <span class="material-symbols-rounded guidelines-search-icon">search</span>
                    <input type="search" id="guidelines-search" 
                           placeholder="Поиск по названию, симптомам..." 
                           autocomplete="off">
                </div>

                <div class="guidelines-sort-controls">
                    <span class="material-symbols-rounded">sort</span>
                    <span class="guidelines-sort-label">Сортировка:</span>
    ...
    renderList(items) {
        ...
        return items.map(item => {
            ...
            const symptomsRaw = Array.isArray(item.symptoms) ? item.symptoms : [];
            const symptoms = symptomsRaw.slice(0, 3).map(s => 
                `<span class="category-badge symptom-badge">${s}</span>`
            ).join('');

            const hasSummary = typeof item.summary === 'string' && item.summary.trim().length > 0;

            return `
                <div class="card card-outlined ripple guideline-item" data-id="${item.id}">
                    <div class="guideline-item-content">
                        <div class="guideline-item-icon">
                            <span class="material-symbols-rounded">${item.icon || 'clinical_notes'}</span>
                        </div>
                        <div class="guideline-item-body">
                            ...
                            <div class="card-subtitle">
                                ...
                                ${symptoms ? `<div class="category-badges symptom-badges">${symptoms}</div>` : ''}
                            </div>
                            ${hasSummary ? `<div class="card-body">${item.summary}</div>` : ''}
                        </div>
                        <span class="material-symbols-rounded guideline-item-arrow">chevron_right</span>
                    </div>
                </div>
            `;
        }).join('');
    ...
    updateUI() {
        const filtered = this.engine.getFiltered();
        const listEl = this.container.querySelector('#guidelines-list');
    ...
    setupEventListeners() {
        this._inputHandler = (e) => {
            if (!e.target.matches('#guidelines-search')) return;
        ...
        this._clickHandler = (e) => {
            // Бонус 2: Клик по бейджу категории (фильтр)
            const categoryBadge = e.target.closest('.category-badge:not(.symptom-badge):not(.critical)');
            if (categoryBadge && e.target.closest('.guideline-item')) {
                e.preventDefault();
                e.stopPropagation();
            ...
            if (guidelineItem && this.container.contains(guidelineItem)) {
                e.preventDefault();
                this.handleGuidelineClick(guidelineItem);
                return;
            }
        };
    ...
    handleGuidelineClick(item) {
        const id = item.dataset.id;
        if (!id) return;
        
        const guideline = this.engine.items.find(g => String(g.id) === String(id));
        
        if (!guideline) {
            console.warn(`⚠️ Протокол с id="${id}" не найден`);
            window.showSnackbar?.('Протокол не найден');
            return;
        }
        
        // ✅ Сохраняем конкретный протокол в историю с source
        try {
            storage.addRecent({
                id: `guideline-${guideline.id}`,
                title: guideline.title,
                subtitle: (guideline._categories || []).slice(0, 2).join(', '),
                icon: guideline.icon || 'clinical_notes',
                section: 'guidelines',
                source: 'guidelines'  // ✅ ДОБАВЛЕНО для цветной иконки
            });
        } catch (e) {
        ...
        // Бонус 1: Переход на страницу протокола (если есть route)
        if (guideline.route) {
            window.location.hash = guideline.route;
        } else {
            window.showSnackbar?.(`${guideline.title} (в разработке)`);
        }
    }
}